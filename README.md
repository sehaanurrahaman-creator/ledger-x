...
contract test, the durable-write contract, the checkpoint contract, the idempotency contract, and
the domain model's and idempotency property suites. The other subcommands, and the `make` targets
that delegate to them:

| Command | Make target | What it does |
| --- | --- | --- |
| `./build.sh` | `make` | lint, compile, the four contracts and both property suites — the verdict |
...
| `./build.sh campaign` | `make campaign` | both property suites at ~10x the campaign CI runs by default |
| `./build.sh compile` | `make compile` | lint and compile, run nothing |
| `./build.sh lint` | `make lint` | the style lint, including the self-test of its three ADR rules |
...
transactions refused atomically, the whole journal, and every balance — and then, against a durable ledger, one
payout posted under an idempotency key, retried (replayed, zero bytes appended), mutated (refused 409), and
retried after a reopen (still the same record) — with Σ balances = 0 asserted before anything is printed, so a
demo that lies cannot get through CI.
...
| [0005 — Idempotency](docs/adr/0005-idempotency.md) | `(merchant, idempotency_key) → response`, with the posting and the binding in **one record** — one append, one fsync, one ack, no between. A retry replays the stored response flagged as a replay, quoting the original record's LSN; a different body under a bound key is **409 Conflict** (not Stripe's 400-replay, which would hide the divergence) at any age; expiry is a 410 refusal, never a re-arming, and clock skew only moves the window. The table lives in memory, in the log and in the checkpoint's v2 state+timing sections — never a fourth place — and `./build.sh` proves it with a fifteen-check contract (including a 100-thread duplicate storm: one posting, 99 replays) and a model-oracle property suite over retries, crashes and recoveries |
...
(`dev.ledgerx.journal`), the idempotency policy and its receipts and refusals
(`dev.ledgerx.idempotency`, ADR 0005), the in-memory double-entry model itself
(`dev.ledgerx.domain`), and the demo (`dev.ledgerx.demo`). `src/test` holds the five-check substrate
contract, the twenty-check durable-write contract — exact frame bytes, 68 single-bit flips, every
truncate offset of a log, the cut-versus-refuse split of the torn-tail rules, per-policy force
counts, and the version byte's whole compatibility story — the thirteen-check checkpoint contract
(each integrity layer refused by its own name, the corrupt-newest fallback, retention and stale
temps, the deletion theorem, the timing section that pairs with the bindings), the fifteen-check
idempotency contract (the receipt, the replay, the 409 at any age, the exact record bytes, the
per-merchant scope, expiry, skew, a hundred-thread duplicate storm, recovery, the checkpoint round
trip, the double-binding refusal), and the two property suites — the domain rules against an
independent `BigInteger` model, and the idempotency theorem against a four-branch model of the ADR
under random interleavings of retries, conflicts, expiries, crashes and skewed clocks.
...
and a recovery with every checkpoint deleted to agree (`PASS 81/81` cycles). And a key now means one thing: `postIdempotent` binds
`(merchant, key)` and its posting in a single record — the duplicate-storm and model-oracle
suites above are its evidence, and [docs/idempotency.md](docs/idempotency.md) is the client's
version of the contract. What is still open on the map: the public API surface (the 409/410
mapping onto HTTP among other things), per-account concurrency, the payout protocol, the
