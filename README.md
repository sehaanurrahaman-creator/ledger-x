...
| `./build.sh replay` | `make replay` | the checkpoint replay harness: every byte offset of every history is a crash — crash, recover, hash |
...
| [0004 — Checkpoint and replay](docs/adr/0004-checkpoint-replay.md) | "Byte-identical" is defined as SHA-256 over a canonical state encoding — accounts in opening order, balances, journal length; LSNs, clocks, randomness and map iteration are banned from state, with the reason for each. A checkpoint is one file — 36-byte header, the canonical bytes, their SHA-256, a CRC — swapped in by write-temp, fsync, atomic rename, fsync-dir; one live checkpoint, the log never truncated behind it. Recovery validates the checkpoint *before* repairing the log: a damaged checkpoint is discarded (renamed, evidence kept), a checkpoint the log cannot prove refuses to open. Proved by crashing at every byte offset of every history |
...
checkpoint contract — both layouts byte for byte, the hash's near-misses, the swap's temp and damage
and refusal matrix — and the nine-check property suite that runs random sequences of valid and
invalid operations against an independent `BigInteger` model of the same rules.
...
invariant violations. And the state is now *proven* identical: `StateHash` defines byte-identical
as SHA-256 over a canonical encoding of the materialized state, `./build.sh replay` crashes at
every byte offset of twelve random histories — checkpoint + tail, full replay from scratch, and
crash-recover-continue must all land on the clean run's exact bytes, and do, at 100% of ~24,000
offsets. What is still open on the map: idempotency semantics, per-account concurrency, the payout
protocol, the benchmark table, and the TLA⁺ spec.
